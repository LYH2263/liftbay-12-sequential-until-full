...
type SeqAssigned = { call_id: number; car_id: number; score: string };
type SeqResult = { assigned: SeqAssigned[]; stopped_call_id: number | null; reason: string };
export default function DispatchPage() {
  ...
  async function runSequence() {
    setMsg(""); setErr(""); setSeq(null);
    try {
      const r = await api<SeqResult>("/dispatch/sequence", { method: "POST" });
      setSeq(r);
      reload();
    } catch (e) { setErr(e instanceof Error ? e.message : String(e)); reload(); }
  }
  ...
  return (<>
    ...
    {seq && (
      <div className="seq-result">
        <div className="ok">
          本次连续派工成功 {seq.assigned.length} 笔：
          {seq.assigned.length
            ? seq.assigned.map(a => ` #${a.call_id}→轿厢${a.car_id}(${a.score})`).join("，")
            : "无"}
        </div>
        {seq.stopped_call_id !== null
          ? <div className="err">停下原因：{seq.reason}（#{seq.stopped_call_id} 及其后呼梯仍 waiting）</div>
          : <div className="ok">{seq.reason || "等待队列已全部派完"}</div>}
      </div>
    )}
